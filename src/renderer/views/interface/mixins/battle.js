...
const Battle = { // 战斗系统
  ...
  methods: {
    ...
    battleStart () {
      this.reset()
      while (this.hero.Health > 0 && this.monster.Health > 0) { // 双方血量归零前持续战斗
        this.turnLogging()
        this.battle()
        this.resultLogging()
      }
    ...
    battle () {
      ...
      let hero = { attribute: this.hero, addition: this.addition, equipped: this.equipped, skill: this.skill }
      let monster = { attribute: this.monster, addition: null, equipped: null, skill: null }
      if (this.hero.Dexterous >= this.monster.Dexterous) { // 判断灵巧，决定谁先手
    ...
    getEquipmentEffect (physicsHarm, magicHarm, self, enemy) {
      ...
      if (self.equipped) {
        self.equipped.length > 1 && self.equipped.sort((a, b) => {
        ...
        enemy.equipped.length > 1 && enemy.equipped.sort((a, b) => {
          return a.Priority - b.Priority
        })
        enemy.equipped.length > 1 && enemy.equipped.forEach(e => {
    ...
    attackLogging (RoleOne, RoleTwo, harm) {
      this.log += `${RoleOne.Name} 攻击了 ${RoleTwo.Name} 1下，造成了${harm}点伤害,`
      if (RoleTwo.Health > 0) {
